...
description: >-
  YuYe 内容自动识别与归档：用户发送文字或链接，识别内容类型，写入
  YuYe/ 七类知识库对应文件夹。触发词：内容归档、整理入库、自动分类、
  YuYe入库、发链接归档、发文字入库、content archiver、知识库归档。
---

# YuYe 内容自动归档 Skill

用户提供**文字或链接**并要求入库、归档、整理、分类时，**立即读取并执行本 Skill**。目标：识别内容 → 选定 YuYe 分类 → 生成结构化笔记 → 写入对应文件夹 → 更新总索引。
...
按主类选模板（路径相对于 `YuYe/`），填充 frontmatter + 正文。**禁止空泛总结**；金句/用户原话必须来自原文。
...
YuYe_category: "01"           # 主类编号
YuYe_subfolder: "lessons-learned"
...
**一键入库流程（`yuye_ingest.py` / `ingest.ps1`）：**
...
3. **分类理由**（1-2 句）
4. **摘要**（3 句话）
5. 总索引已更新：`YuYe/_content_index.md`
